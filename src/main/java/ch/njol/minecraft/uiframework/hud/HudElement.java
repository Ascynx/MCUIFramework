...
public abstract class HudElement {
	 ...
	 */
	public void drawOutlinedText(DrawContext drawContext, String text, int x, int y, int color) {
		//TODO might want to check if the shadow boolean does the same, if so then deprecate this method and refer to DrawContext#drawText
		MatrixStack matrices = drawContext.getMatrices();
		matrices.push();
		matrices.translate(0, 0, 1);
		drawContext.drawText(client.textRenderer, text, x - 1, y, 0, false);
		drawContext.drawText(client.textRenderer, text, x, y - 1, 0, false);
		drawContext.drawText(client.textRenderer, text, x + 1, y, 0, false);
		drawContext.drawText(client.textRenderer, text, x, y + 1, 0, false);
	...
	public static void drawTextureSmooth(DrawContext drawContext, float x, float y, float width, float height) {
		drawTexturedQuadSmooth(drawContext.getMatrices().peek().getPositionMatrix(), x, y, x + width, y + height, 0, 0, 0, 1, 1);
	}

	public static void drawTextureSmooth(DrawContext drawContext, float x, float y, float width, float height, float u0, float u1, float v0, float v1) {
		drawTexturedQuadSmooth(drawContext.getMatrices().peek().getPositionMatrix(), x, y, x + width, y + height, 0, u0, v0, u1, v1);
	}
	...
	/**
	 * @deprecated use {@link DrawContext#drawSprite(int, int, int, int, int, Sprite)} instead.
	**/
	public static void drawSprite(DrawContext drawContext, Sprite sprite, float x, float y, float width, float height) {
		//drawContext.drawSprite((int) x, (int) y, 0, (int) width, (int) height, sprite);
		RenderSystem.setShaderTexture(0, sprite.getAtlasId());
		drawTexturedQuadSmooth(drawContext.getMatrices().peek().getPositionMatrix(), x, y, x + width, y + height, 0, sprite.getMinU(), sprite.getMinV(), sprite.getMaxU(), sprite.getMaxV());
	}
	 ...
	 */
	public static void drawPartialSprite(DrawContext drawContext, Sprite sprite, float x, float y, float width, float height, float cutX0, float cutY0, float cutX1, float cutY1) {
	...
	public static void drawRepeatingSprite(DrawContext drawContext, Sprite sprite, float x, float y, float width, float height, float xRepetitions, float yRepetitions) {
